...
def events():
    if "user_id" not in session:
        flash("Você precisa estar logado para acessar os eventos.", "warning")
        return redirect("/login")
    
    events = Event.query.all()
    
    # Para cada evento, buscamos os participantes
    event_participants = {}
    for event in events:
        participants = [user_event.user for user_event in event.user_events]
        event_participants[event.id] = participants

    return render_template("events.html", events=events, event_participants=event_participants)


@app.route("/create_event", methods=["GET", "POST"])
def create_event():
    if "user_id" not in session:
        flash("Você precisa estar logado para criar um evento.", "warning")
        return redirect("/login")
    ...
    if request.method == "POST":
        ...
        # Remover a parte que lida com a data
        new_event = Event(name=name, description=description, user_id=user_id)
...
def join_event(event_id):
    if "user_id" not in session:
        flash("Você precisa estar logado para se inscrever em um evento.", "warning")
        return redirect("/login")
    
    user_id = session["user_id"]
    
    # Verificar se o usuário já está inscrito no evento
    existing_user_event = UserEvent.query.filter_by(user_id=user_id, event_id=event_id).first()
    ...
    flash("Você se inscreveu no evento com sucesso!", "success")
    return redirect("/events")
